    ...
    <article className="bg-zinc-900 border border-yellow-600/20 rounded-2xl p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {(() => {
            const hasReturn = !!(b.return_date || b.return_time || b.return_pickup || b.return_dropoff);
            const typeLabel =
              b.trip_type === "hourly" ? "hourly" : hasReturn ? "round trip" : "transfer";
            return (
              <>
                <p className="text-[11px] tracking-[0.14em] uppercase text-zinc-500 mt-3">Outbound</p>
                <div className="mt-1 font-medium">{formatDateTime(b.ride_date, b.ride_time) || "Date TBD"}</div>
                <p className="text-sm text-zinc-400 mt-1">{b.pickup || "—"}</p>
                <p className="text-sm text-zinc-400">→ {b.dropoff || "—"}</p>
                {b.flight_number ? <p className="text-xs text-zinc-500 mt-1">Flight {b.flight_number}</p> : null}
                {hasReturn && (
                  <>
                    <p className="text-[11px] tracking-[0.14em] uppercase text-zinc-500 mt-4">Return</p>
                    <div className="mt-1 font-medium">
                      {formatDateTime(b.return_date, b.return_time) || "Return time TBD"}
                    </div>
                    <p className="text-sm text-zinc-400 mt-1">{b.return_pickup || b.dropoff || "—"}</p>
                    <p className="text-sm text-zinc-400">→ {b.return_dropoff || b.pickup || "—"}</p>
                    {b.return_flight_number ? (
                      <p className="text-xs text-zinc-500 mt-1">Flight {b.return_flight_number}</p>
                    ) : null}
                  </>
                )}
                <p className="text-xs text-zinc-500 mt-3">
                  {vehicleLabel[b.vehicle] || b.vehicle} · {typeLabel} · {money(b.amount_cents)}
                </p>
              </>
            );
          })()}
