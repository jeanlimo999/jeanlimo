...
export default function RideCard({
  ...
  const initial = String(b.driverName || "J").trim().charAt(0).toUpperCase();
      ...
      <div className="mt-3 rounded-2xl border border-white/10 bg-[#141414] p-4">
        <div className="flex gap-3">
          <div className="w-14 text-center">
            ...
            <div className="text-[10px] text-zinc-500">{d.ride_date?.slice(0,4) || ""}</div>
          </div>
          <div className="flex-1">
            <div className="text-sm font-medium">{shortAddr(b.pickup)} → {shortAddr(b.dropoff)}</div>
            <div className="mt-1 text-xs text-zinc-400">{prettyTime(b.ride_time)} · {vehicleLabel(b.vehicle)}</div>
            {onAgain && <button onClick={onAgain} className="mt-3 w-full rounded-xl border border-[#d8b56b]/50 py-2 text-sm text-[#e8d3b0]">Book Again</button>}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="mt-3 rounded-[22px] border border-white/10 bg-[#121212] p-4">
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full bg-[#1a3d32] px-2 py-1 text-[10px] tracking-widest text-[#7DCFB6]">{status.toUpperCase()}</span>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl bg-[#0d0d0d] p-3">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#2a2418] text-[#e8d3b0]">{initial}</div>
          <div>
            <div className="text-[11px] text-zinc-500">Your Chauffeur</div>
            <div className="font-medium">{b.driverName || "Assigned at dispatch"}</div>
          </div>
        </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button onClick={onChange} className="rounded-xl border border-[#d8b56b]/40 py-3 text-sm">Request Change</button>
        {b.driverPhone ? (
          <a href={sms(b.driverPhone)} className="rounded-xl bg-gradient-to-r from-[#e3c17a] to-[#bc8d3d] py-3 text-center text-sm font-semibold text-black">Message Driver</a>
        ) : (
          <a href="sms:+12819170085" className="rounded-xl bg-gradient-to-r from-[#e3c17a] to-[#bc8d3d] py-3 text-center text-sm font-semibold text-black">Message Dispatch</a>
        )}
      </div>

      <div className="mt-4 space-y-2 text-sm text-zinc-300">
        <div className="flex justify-between"><span className="text-zinc-500">Flight</span><span>{b.flight_number || "—"}</span></div>
        <div className="flex justify-between"><span className="text-zinc-500">Confirmation</span><span>{b.confirmation}</span></div>
        <button onClick={onCancel} className="w-full pt-2 text-left text-xs text-red-300">Cancel ride</button>
      </div>
    </div>
  );
}
