    ...
    <div className="bg-zinc-900/90 backdrop-blur border border-yellow-600/20 rounded-2xl p-6 md:p-8 card-glow">
      ...
      {quotes && showBooking && selectedQuote && (
        <div className="mt-6 p-5 bg-zinc-800/80 border border-yellow-600/30 rounded-xl space-y-4">
          ...
          <div>
            <p className="text-sm text-zinc-300 mb-2">Add a tip (optional)</p>
            <div className="grid grid-cols-2 gap-2">
              {[0, 10, 15, 20, 25].map((pct) => {
                ...
                return (
                  ...
                  >
                    <div className="font-semibold">{pct === 0 ? "No tip" : `${pct}%`}</div>
            ...
            <div className="mt-3 text-sm text-zinc-400 space-y-1">
              <div className="flex justify-between"><span>Ride</span><span>${selectedQuote.price.toFixed(2)}</span></div>
              <div className="flex justify-between"><span>Tip</span><span>${(Math.round(selectedQuote.price * tipPct) / 100).toFixed(2)}</span></div>
              <div className="flex justify-between text-yellow-500 font-semibold"><span>Pay now</span><span>${(selectedQuote.price + Math.round(selectedQuote.price * tipPct) / 100).toFixed(2)}</span></div>
            </div>
          </div>
