...
export default function QuoteWidget() {
  ...
  const handlePay = async () => {
    ...
    try {
      const tipAmount = Math.round(selectedQuote.price * tipPct) / 100;
  ...
  return (
    <div className="bg-zinc-900/90 backdrop-blur border border-yellow-600/20 rounded-2xl p-6 md:p-8 card-glow">
      ...
      {quotes && showBooking && selectedQuote && (
        <div className="mt-4 p-3 bg-zinc-800/80 border border-yellow-600/30 rounded-xl space-y-2">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-sm text-zinc-300">
                {VEHICLES.find((v) => v.id === vehicle)?.name}
              </div>
              <div className="text-[11px] text-zinc-500">{selectedQuote.breakdown}</div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-[11px] uppercase tracking-wider text-zinc-400">Total due</div>
              <div className="text-xl font-serif text-yellow-500 leading-tight">
                ${(selectedQuote.price + Math.round(selectedQuote.price * tipPct) / 100).toFixed(2)}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="col-span-2">
              <label className="block text-[11px] text-zinc-400 mb-0.5">Full Name *</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-zinc-700 border border-zinc-600 rounded-md px-3 py-1.5 text-sm" />
            </div>
            <div>
              <label className="block text-[11px] text-zinc-400 mb-0.5">Phone *</label>
              <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full bg-zinc-700 border border-zinc-600 rounded-md px-3 py-1.5 text-sm" />
            </div>
            <div>
              <label className="block text-[11px] text-zinc-400 mb-0.5">Email *</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-zinc-700 border border-zinc-600 rounded-md px-3 py-1.5 text-sm" />
            </div>
          </div>

          <div>
            <p className="text-[11px] text-zinc-400 mb-1">Add a tip</p>
            <div className="grid grid-cols-5 gap-1">
              {[0, 10, 15, 20, 25].map((pct) => {
                ...
                return (
                  ...
                  >
                    <div className="text-[11px] font-semibold leading-tight">{pct === 0 ? "No tip" : `${pct}%`}</div>
                    <div className="text-[10px] leading-tight">${amt.toFixed(0)}</div>
                  </button>
                );
              })}
            </div>
          ...
          <button
            onClick={handlePay}
            disabled={loading}
            className="w-full py-2.5 bg-yellow-500 hover:bg-yellow-400 disabled:opacity-60 text-zinc-900 font-semibold rounded-lg transition text-sm"
          >
            {loading ? "Redirecting to Stripe…" : "Pay Securely with Stripe"}
          </button>
          <button onClick={() => setShowBooking(false)} className="w-full py-1 text-xs text-zinc-400 hover:text-white">
            ← Back to vehicles
          </button>
        </div>
      )}
