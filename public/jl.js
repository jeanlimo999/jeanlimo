(function (w) {
  ...
  function messages(job, driver) {
    ...
    return {
      ...
      on_location: "Hi " + guest + ", this is " + dname + " with Jean Limo. I'm on location and ready for you" + pu + ".",
  ...
  async function listJobs() {
    const db = sb();
    if (db) {
      const { data, error } = await db.from("bookings").select("*, clients(full_name, phone, email)").not("status", "eq", "cancelled").order("ride_date", { ascending: true });
      if (!error && data) return data.flatMap(row => rowsToJobs(row));
    }
  ...
  function rowsToJobs(row) {
    ...
    const base = {
      bookingId: row.id, source: "Jean Limo",
      guestName: guest.full_name || "Guest", guestPhone: guest.phone || "",
      vehicle: row.vehicle, assignedDriverId: row.assigned_driver_id || "",
      tripStatus: row.trip_status || row.status || "confirmed",
      last_lat: row.last_lat, last_lng: row.last_lng,
      amountCents: row.amount_cents || 0
    };
    const jobs = [{
      ...base, id: row.confirmation, conf: row.confirmation,
      rideDate: row.ride_date || "", rideTime: row.ride_time || "",
      when: [row.ride_date, row.ride_time].filter(Boolean).join(" "),
      pickup: row.pickup, dropoff: row.dropoff, live_leg: "outbound"
    }];
    if (row.return_date || row.return_pickup) {
      jobs.push({
        ...base, id: row.confirmation + "-R", conf: row.confirmation + " return",
        rideDate: row.return_date || "", rideTime: row.return_time || "",
        when: [row.return_date, row.return_time].filter(Boolean).join(" "),
        pickup: row.return_pickup || row.dropoff,
        dropoff: row.return_dropoff || row.pickup, live_leg: "return"
      });
    }
    return jobs;
  }
  ...
  async function assignDriver(job, driverId) {
    ...
    if (db && job.bookingId && !String(job.bookingId).startsWith("local")) {
      await db.from("bookings").update({ assigned_driver_id: driverId || null, trip_status: driverId ? "assigned" : "confirmed", updated_at: new Date().toISOString() }).eq("id", job.bookingId);
    }
  }
  async function setTripStatus(job, status) {
    const db = sb();
    if (db && job.bookingId && !String(job.bookingId).startsWith("local")) {
      await db.from("bookings").update({ trip_status: status, live_leg: job.live_leg || "outbound", updated_at: new Date().toISOString() }).eq("id", job.bookingId);
    }
  }
